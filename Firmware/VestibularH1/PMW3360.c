...
{
	/* Shutdown */
	optical_tracking_write_register_pmw3360_0(REG_Shutdown, 0xB6);
	_delay_ms(300);
	...
	/* Upload firmware */
	if (prodID == 0x42)
		upload_the_firmware_pmw3360_0();
	
	/* Check all signatures including SROM */
	if (prodID == 0x42)
		if (check_signatures_pmw3360_0(true) == false)
			return false;
...
{
	/* Shutdown */
	optical_tracking_write_register_pmw3360_1(REG_Shutdown, 0xB6);
	_delay_ms(300);
	...
	/* Upload firmware */
	if (prodID == 0x42)
	upload_the_firmware_pmw3360_1();
	
	/* Check all signatures including SROM */
	if (prodID == 0x42)
	if (check_signatures_pmw3360_1(true) == false)
	return false;
...
uint8_t check_signatures_pmw3360_0(bool check_srom)
{
	...
	uint8_t sromID = optical_tracking_read_register_pwm3360_0(REG_SROM_ID);
	
	/* Check product ID */
	if (productID != 0x42 && productID != 0x47)
	return false;
	
	if (productID == 0x42)
	if (invProductID != 0xBD)
	return 0;
	
	if (productID == 0x47)
	if (invProductID != 0xB8)
	return 0;
	
	/* Read the SROM_ID register to verify the firmware ID */
	if (check_srom)
	if (sromID != 0x04)
	return 0;
...
uint8_t check_signatures_pmw3360_1(bool check_srom)
{
	...
	uint8_t sromID = optical_tracking_read_register_pwm3360_1(REG_SROM_ID);
...
void upload_the_firmware_pmw3360_0(void)
{
	...
	optical_tracking_write_register_pmw3360_0(REG_Config2, 0x00);

	/* write 0x1d in SROM_enable reg for initializing */
	optical_tracking_write_register_pmw3360_0(REG_SROM_Enable, 0x1d);

	/* Wait for more than one frame period */
	_delay_ms(10); // Assume that the frame rate is as low as 100fps... even if it should never be that low

	/* Write 0x18 to SROM_enable to start SROM download */
	optical_tracking_write_register_pmw3360_0(REG_SROM_Enable, 0x18);
	
	spi_start_flow0();
	_delay_us(1);
	
	spi_tx_byte_flow0(REG_SROM_Load_Burst | 0x80); // Write burst destination address
	_delay_us(25);
	...
	/* Write 0x00 for wired mouse (0x20 for wireless mouse design) to disable rest modes */
	optical_tracking_write_register_pmw3360_0(REG_Config2, 0x00);  
}
...
{
	/* Write 0 to Rest_En bit of Config2 register to disable Rest mode */
	optical_tracking_write_register_pmw3360_1(REG_Config2, 0x00);

	/* write 0x1d in SROM_enable reg for initializing */
	optical_tracking_write_register_pmw3360_1(REG_SROM_Enable, 0x1d);

	/* Wait for more than one frame period */
	_delay_ms(10); // Assume that the frame rate is as low as 100fps... even if it should never be that low

	/* Write 0x18 to SROM_enable to start SROM download */
	optical_tracking_write_register_pmw3360_1(REG_SROM_Enable, 0x18);
	
	spi_start_flow1();
	_delay_us(1);
	
	spi_tx_byte_flow1(REG_SROM_Load_Burst | 0x80); // Write burst destination address
	_delay_us(25);
...
{
	if (cpi > PMW3360_MAX_CPI) cpi = PMW3360_MAX_CPI;
	if (cpi < PMW3360_MIN_CPI) cpi = PMW3360_MIN_CPI;
	
	/* Convert CPI to register value */
	uint8_t value = (cpi / 100) - 1;
	
	optical_tracking_write_register_pmw3360_0(REG_Config1, value);
}
void set_cpi_pmw3360_1 (uint32_t cpi)
{
	...
	optical_tracking_write_register_pmw3360_1(REG_Config1, value);
}

uint16_t get_cpi_pmw3360_0 (void)
{
	uint8_t value = optical_tracking_read_register_pwm3360_0(REG_Config1);
	
	return (uint16_t) (value + 1) * 100.0;
}
uint16_t get_cpi_pmw3360_1 (void)
{
	uint8_t value = optical_tracking_read_register_pwm3360_1(REG_Config1);
...
{  
	if(!_inBurst)
	{
		...
		SPIC_DATA = REG_Motion_Burst | 0x80;
		SPID_DATA = REG_Motion_Burst | 0x80;
	...
	_delay_us(2);
		...
		/* Starts readings */
	SPIC_DATA = REG_Motion_Burst;
	SPID_DATA = REG_Motion_Burst;
	loop_until_bit_is_set(SPIC_STATUS, SPI_IF_bp);
	loop_until_bit_is_set(SPID_STATUS, SPI_IF_bp);	
		
	/* Get isMotion and isOnSurface */
	SPIC_DATA = 0;
	SPID_DATA = 0;	
	loop_until_bit_is_set(SPIC_STATUS, SPI_IF_bp);
	loop_until_bit_is_set(SPID_STATUS, SPI_IF_bp);	
	motion_flow0->isMotion = (SPIC_DATA & 0x80) != 0;
	motion_flow0->isOnSurface = (SPIC_DATA & 0x08) == 0;
	motion_flow1->isMotion = (SPID_DATA & 0x80) != 0;
	motion_flow1->isOnSurface = (SPID_DATA & 0x08) == 0;
	...
	for (uint8_t i = 0; i < 5; i++)
	{
		...
		loop_until_bit_is_set(SPIC_STATUS, SPI_IF_bp);
		*(((uint8_t*)(motion_flow0))+(i+2)) = SPIC_DATA;		
		loop_until_bit_is_set(SPID_STATUS, SPI_IF_bp);
		*(((uint8_t*)(motion_flow1))+(i+2)) = SPID_DATA;
	}
